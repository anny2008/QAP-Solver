...
std::vector<double> ColumnGenSolver::buildPhi(const Problem& P) {
    ...
    // // export phi to file for debugging
    // auto filename = "instance03_phi_matrix.txt"; 
    // std::cout << "[CG] Exporting phi matrix to " << filename << "\n";
    // std::ofstream phi_file(filename);
    // for (int i=0;i<n;++i) {
    //     for (int u=0;u<m;++u) {
    //         for (int j=0;j<n;++j) {
    //             for (int v=0;v<m;++v) {
    //                 if(phi[(((i*m)+u)*n + j)*m + v] > 0.0)
    //                     phi_file << "(" << i << "," << u << "," << j << "," << v << "): " << phi[(((i*m)+u)*n + j)*m + v] << "\n";
    //             }
    //         }
    //     }
    // }
...
void find_a_valid_dual_solution_restricted2_Gurobi(const Problem& problem, DualVector& duals, std::vector<QuadKey>& tight_columns) {
    ...
    for (int i=0;i<n;++i) for (int j=i+1;j<n;++j) for (int u=0;u<m;++u) for (int v=0;v<m;++v) {
        if (u==v) continue;
        double rhs = problem.D[i][j] * problem.F[u][v] + problem.D[j][i] * problem.F[v][u];
        if(rhs < 1e-6) continue; // skip if rhs is too small
...
void read_Omega0_from_file(const std::string& filename, std::vector<QuadKey>& omega0) {
    std::ifstream infile(filename);
    if (infile.is_open()) {
        int i, u, j, v;
        while (infile >> i >> u >> j >> v) {
...
)
{
    ...
    std::cout << "[CG] Reading initial columns from file " << positive_y_file << "\n";
    read_Omega0_from_file(positive_y_file, Omega0);
    ...
    while (true) {
        ...
        if (!solved) {
            ...
            if (!added_ls) {
                ...
                std::cerr << "[CG] Error: RMP solving status : " << rmp.model().get(GRB_IntAttr_Status) << ".\n";
                break;
            }
        }
        ++iteration;
