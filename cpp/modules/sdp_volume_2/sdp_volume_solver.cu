...
SDPVolumeHooks2::SDPVolumeHooks2(const Problem& data, VOL_problem& vol_prob, std::string eigen_solver)
    : SDPVolumeHooks(data, vol_prob), n(data.n), alpha(data.n), eigen_solver(eigen_solver), gpu_pipeline()
{

    // --- 1. Flatten qap_data.D (n x n Matrix) ---
    std::vector<double> h_D_flat(n * n);
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            h_D_flat[i * n + j] = qap_data.D[i][j];
        }
    }

    // --- 2. Flatten qap_data.F (n x n Matrix) ---
    std::vector<double> h_F_flat(n * n);
    for (int u = 0; u < n; ++u) {
        for (int v = 0; v < n; ++v) {
            h_F_flat[u * n + v] = qap_data.F[u][v];
        }
    }
    d_D = nullptr;
    d_F = nullptr;
    ...
    auto n2 = n * n;
    auto pi_size = 2*n + 2*n*n*n + (n2 * (n2 + 1)) / 2;
    // Assuming 'pi_size' is the total length of your 1D 'pi' array
    cudaMalloc((void**)&d_pi, sizeof(double) * pi_size); 

    cudaMemcpy(d_D, h_D_flat.data(), sizeof(double) * n * n, cudaMemcpyHostToDevice);
    cudaMemcpy(d_F, h_F_flat.data(), sizeof(double) * n * n, cudaMemcpyHostToDevice);


    int N = n * n;
    std::cout << "Allocating GPU memory for matrices of size: " << N << " x " << N << std::endl;
    d_A = nullptr;      cudaMalloc((void**)&d_A, sizeof(double) * N * N);
    d_W = nullptr;      cudaMalloc((void**)&d_W, sizeof(double) * N);
    d_v_gpu = nullptr;  cudaMalloc((void**)&d_v_gpu, sizeof(double) * N);
    d_X = nullptr;      cudaMalloc((void**)&d_X, sizeof(double) * N * N);
    d_info = nullptr;      cudaMalloc((void**)&d_info, sizeof(int));
    std::cout << "GPU memory allocation completed." << std::endl;
    cusolverDnHandle_t cusolverH = NULL;
    cusolverDnCreate(&cusolverH);
    std::cout << "Initialized cuSolver handle." << std::endl;

    gpu_pipeline.initialize(n, pi_size, qap_data.D, qap_data.F);
...
int SDPVolumeHooks2::solve_subproblem_2(const VOL_dvector& pi,
    ...
    {
        auto vio_u_offset = 0;
        auto vio_i_offset = vio_u_offset + n;
        auto vio_iuv_offset = vio_i_offset + n;
        auto vio_iuj_offset = vio_iuv_offset + n*n*n;
        auto vio_iujv_offset = vio_iuj_offset + n*n*n;

        gpu_pipeline.execute_iteration_step(
            pi.v, alpha, 
            lcost, pcost, 
            vio.v + vio_u_offset, vio.v + vio_i_offset,
            vio.v + vio_iuj_offset, vio.v + vio_iuv_offset,
            vio.v + vio_iujv_offset
        );
    }
...
SDPVolumeHooks2::~SDPVolumeHooks2() {
    cusolverDnDestroy(cusolverH);
    cudaFree(d_A); cudaFree(d_W); cudaFree(d_v_gpu); cudaFree(d_X); cudaFree(d_info);
}
...
SDPVolumeHooks6::SDPVolumeHooks6(const Problem& data, VOL_problem& vol_prob, std::string eigen_solver)
    : SDPVolumeHooks(data, vol_prob), n(data.n), alpha(data.n - 1), eigen_solver(eigen_solver)
{
    W = build_Vhat(n);
    ...
    Eigen::MatrixXd A = convert_nested_vector_to_eigen(data.D);
    Eigen::MatrixXd B = convert_nested_vector_to_eigen(data.F);
    //  L = [0 0; 0 B kron A] -> Size: (n^2+1 x n^2+1)
    L = Eigen::MatrixXd::Zero(n*n+1, n*n+1);
    L.block(1, 1, n*n, n*n) = kroneckerProduct(B, A);
...
/*
    ...
    * subject to:
    *   X is PSD, trace X = alpha
    *   Y is satisfying : symmetric constraints, gangster constraints, and non-negativity constraints
    * compute min eigenvalue of C - pi lambda
    * and corresponding eigenvector v
    * X* = alpha*vv^T
    * lcost = lambda*alpha + <C + pi,Y*>
    * pcost = <C, X*> = <C, vv^T>
    * vio = X* - Y*
    * pi [n*n*n*n]
*/    
int SDPVolumeHooks6::solve_subproblem_6(const VOL_dvector& pi,
    ...
    {
        int pi_iuv_offset = 0;
        int pi_iuj_offset = n*n*n;
        int pi_iujv_offset = 2*n*n*n;
        Eigen::MatrixXd Pi = Eigen::Map<const Eigen::MatrixXd>(pi.v, n*n+1, n*n+1);

        //  solve X subproblem: min <C + pi, X> s.t. X is PSD, trace X = alpha
        auto C_plus_pi = L + Pi;
        Eigen::MatrixXd WCW = W.transpose() * C_plus_pi * W;
        ...
        if (eigen_solver == "power_iteration")
        {
            ...
            // if (!converged) 
            {
                Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd> eigensolver(WCW);
                Eigen::Index minIndex;
                lambda = eigensolver.eigenvalues().minCoeff(&minIndex);
                v = eigensolver.eigenvectors().col(minIndex);
            }
        ...
        {
            Spectra::DenseSymMatProd<double> op(WCW);
            int matrix_dim = WCW.rows();
            int ncv = std::min(2, matrix_dim);
            if (ncv <= 2) ncv = 3;
            ...
            // 2. We keep the fallback checking 'nconv > 0' because as long as it has computed 
            //    at least 1 rough estimate, it's valid for subproblem optimization bounds!
            int nconv = eigs.compute(Spectra::SortRule::LargestAlge, 20, 1e-5);
        ...
        auto R = alpha*(v * v.transpose());
        double trace_R = R.trace();
        Eigen::MatrixXd  X = W * R * W.transpose();

        // solve Y subproblem: min <C + pi, Y> s.t. Y is symmetric, gangster, and non-negative
        auto C_minus_pi = L - Pi;
        Eigen::MatrixXd Y = Eigen::MatrixXd::Zero(n*n + 1, n*n + 1);
        Y(0,0) = 1.0;
        // if C + pi <= 0 then Y = 1
        for (int i = 0; i < n; ++i)
            for (int u = 0; u < n; ++u)
                for (int j = i; j < n; ++j) 
                    for (int v = 0; v < n; ++v)
                    {
                        auto is_gangster = (i != j && u == v) || (i == j && u != v);
                        if (is_gangster)continue;
                        if (C_minus_pi(i*n + u+1, j*n + v+1) <= 0)
                        {
                            Y(i*n + u+1, j*n + v+1) = 1.0;
                            Y(j*n + v+1, i*n + u+1) = 1.0; // ensure symmetry
                        }
                    }
        
        auto vio_matrix = (X - Y).eval();
        // compute pcost = <C, X> + <C, Y>
        pcost = (L * X).trace() + (L * Y).trace();
        lcost = pcost + Pi.cwiseProduct(vio_matrix).sum();
        std::copy(vio_matrix.data(), vio_matrix.data() + vio_matrix.size(), vio.v);
...
void set_up_vol_problem_2(VOL_problem &vol_problem, const Problem &qap_data, const FixedVariables &fixed) {
    ...
    int total_size = 2 * n + 2 * n * n * n + n2*n2;
