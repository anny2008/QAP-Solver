...
public:
    ...
    // Build all row shells upfront (as Python ensure_all_rows)
    void ensureAllRows();
    ...
    double getRealObjectiveValue() const;
    double getRowDualC1(int i, int j) const;
    double getRowDualC2(int u, int v) const;
    double getRowDualC3(int u, int j) const;
    double getRowDualC4(int i, int u, int j) const;
    double getRowDualC5(int i, int u, int v) const;
    ...
    void addAllTplusTminus(); // add all t_plus and t_minus variables for dual box restriction (for stabilization)
    // Update stabilization coefficients based on current duals (for stabilized column generation)
    void updateStabilizationCoefficients(const DenseDuals& hat_pi, const DenseDuals& delta);
...
// private:
    ...
    IloRange& ensureC4(int i, int u, int j);
    IloRange& ensureC5(int i, int u, int v);
    ...
    std::unordered_map<TripleKey, IloRange, TripleKeyHash> c4_map;
    ...
    //  dual box restriction for stabilization
    std::unordered_map<PairKey, IloNumVar, PairKeyHash> t_plus_C1;
    std::unordered_map<PairKey, IloNumVar, PairKeyHash> t_minus_C1;

    std::unordered_map<PairKey, IloNumVar, PairKeyHash> t_plus_C2;
    std::unordered_map<PairKey, IloNumVar, PairKeyHash> t_minus_C2;
    
    std::unordered_map<PairKey, IloNumVar, PairKeyHash> t_plus_C3;
    std::unordered_map<PairKey, IloNumVar, PairKeyHash> t_minus_C3;

    std::unordered_map<TripleKey, IloNumVar, TripleKeyHash> t_plus_C4;
    std::unordered_map<TripleKey, IloNumVar, TripleKeyHash> t_minus_C4;

    std::unordered_map<TripleKey, IloNumVar, TripleKeyHash> t_plus_C5;
    std::unordered_map<TripleKey, IloNumVar, TripleKeyHash> t_minus_C5;
    ...
    std::unordered_map<QuadKey, IloCplex::BasisStatus, QuadKeyHash> lastIterBasicStatus; // store basis status of variables for debugging
};