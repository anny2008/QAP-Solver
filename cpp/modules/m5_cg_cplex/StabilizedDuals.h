...
struct DenseDuals {
    ...
    std::vector<double> C4;      // n*m*n
    std::vector<double> C5;      // n*m*m
    ...
    inline int idxC4(int i,int u,int j) const { return ( (i*m + u)*n + j ); }
    inline int idxC5(int i,int u,int v) const { return ( (i*m + u)*m + v ); }
    ...
    inline double getC4(int i,int u,int j) const { return C4[idxC4(i,u,j)]; }
    inline double getC5(int i,int u,int v) const { return C5[idxC5(i,u,v)]; }

    // Capture current duals from the RMP
    void captureFrom(const IncrementalRMP& rmp) {
        assert(n>0 && m>0);
        for (int i=0;i<n;++i) for (int j=0;j<n;++j) if(i < j) C1[idxC1(i,j)] = rmp.getRowDualC1(i,j);
        for (int u=0;u<m;++u) for (int v=0;v<m;++v) if(u < v) C2[idxC2(u,v)] = rmp.getRowDualC2(u,v);
        ...
        for (int i=0;i<n;++i) for (int u=0;u<m;++u) for (int j=0;j<n;++j)
            if (i!=j) C4[idxC4(i,u,j)] = rmp.getRowDualC4(i,u,j);
        for (int i=0;i<n;++i) for (int u=0;u<m;++u) for (int v=0;v<m;++v)
            if (u!=v) C5[idxC5(i,u,v)] = rmp.getRowDualC5(i,u,v);
    }
    ...
    void emaUpdate(const DenseDuals& cur, double lambda) {
        ...
        blend(C4, cur.C4); blend(C5, cur.C5);
    }
};
...
public:
    ...
    inline double getRowDualC4(int i,int u,int j) const { return du.getC4(i,u,j); }
    inline double getRowDualC5(int i,int u,int v) const { return du.getC5(i,u,v); }
