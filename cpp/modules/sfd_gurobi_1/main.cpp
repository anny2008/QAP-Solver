...
int main(int argc, char* argv[]) {
    ...
    if(problem.m < problem.n) {
        // padding
        std::cout << "Padding problem from size " << problem.m << " to " << problem.n << " (square flow matrix)." << std::endl;
        auto newF = std::vector<std::vector<double>>(problem.n, std::vector<double>(problem.n, 0.0));
        for (int u = 0; u < problem.m; ++u) {
            for (int v = 0; v < problem.m; ++v) {
                if (u < problem.n && v < problem.n) {
                    newF[u][v] = problem.F[u][v];
                }
            }
        }
        problem.m = problem.n;
