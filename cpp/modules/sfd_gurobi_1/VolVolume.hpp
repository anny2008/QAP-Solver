   ...
   lfactor(const VOL_parms &parm, const double lambda, const int iter)
   {
      ...
      case yellow:
         ...
         if (cons >= parm.yellowtestinvl)
         {
            ...
            lambdafactor = 1.1;
            if (parm.printflag & 2)
               std::cout << "\r **** increasing lamda to " << lambda * lambdafactor << " *****\n\n" << std::flush;
         }
         break;
      ...
      case red:
         ...
         if (cons >= parm.redtestinvl && lambda > eps)
         {
            ...
            lambdafactor = 0.67;
            if (parm.printflag & 2)
               std::cout << "\r **** decreasing lamda to " << lambda * lambdafactor << " *****\n\n" << std::flush;
         }
         break;
      }
      return lambdafactor;
...
public:
   ...
   double UB; // Upper bound for the problem, if known. If not known, set to DBL_MAX
