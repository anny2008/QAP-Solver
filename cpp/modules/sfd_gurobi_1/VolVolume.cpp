...
VOL_problem::read_params(const char* filename) 
{
   ...
   if (!infile) {
      std::cout << "\rFailure to open file: " << filename  << std::flush;
      abort();
   }
...
int VOL_problem::solve(VOL_user_hooks& hooks, const bool use_preset_dual) 
{
   ...
   for (iter_ = 1; iter_ <= parm.maxsgriters; ++iter_) {  // main iteration
      ...
      if(dual.lcost > hooks.UB*2) {
         // If the dual cost exceeds the known upper bound, we can terminate early
         if (parm.printflag & 2) std::cout << "\rDual cost exceeds upper bound, primal is infeasible" << std::endl;
	      break;
      }
