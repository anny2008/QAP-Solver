...
private:
    ...
    // internal maps
    std::vector<IloRange> c1_map; // i
    std::vector<IloRange> c2_map; // u
    std::vector<IloRange> c3_map; // (i,u,j)
    std::vector<IloRange> c4_map; // (i,u,v)

    std::vector<IloNumVar> x_index; // (i,u)
    std::vector<IloNumVar> y_index; // canonical (i,u,j,v)
