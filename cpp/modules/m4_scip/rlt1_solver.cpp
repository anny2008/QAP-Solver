/**
 * M4 SCIP QAP Solver - Main Implementation
 * 
 * Implements the Reformulation-Linearization Technique (M4) for Quadratic Assignment Problem
 * using SCIP 9.2.0.
 * 
...
#include "../m4_volume/m4_volume_solver.hpp"
...
/**
 * Create M4 x variables: x[i,u] ∈ {0,1} for facility i at location u
 */
...
/**
 * Create M4 y variables: y[i,u,j,v] ∈ [0,1] (continuous) for linearization
 */
        ...
        for (int u = 0; u < n; ++u) {
            for (int j = 0; j < n; ++j) {
                for (int v = 0; v < n; ++v) {
                    ...
                    // CONTINUOUS variables for M4 linearization
...
/**
 * Add M4 linking constraints:
 * - Σ_uv y[i,u,j,v] = 1  (for all ij)
 * - Σ_ju y[i,u,j,v] = 1  (for all iv)
 * - Σ_iv y[i,u,j,v] = 1  (for all ju)
 * - Σ_ij y[i,u,j,v] = 1  (for all uv)
 
 */
void add_m4_constraints(SCIP* scip, int n, const XVarMap& x, const YVarMap& y) {
    // Σ_uv y[i,u,j,v] = 1 for all i,j
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            SCIP_CONS* cons = nullptr;
            char consname[256];
            snprintf(consname, 255, "link1_%d_%d", i, j);
    ...
    // Σ_ju y[i,u,j,v] = 1 for all i,v
    for (int i = 0; i < n; ++i) {
        for (int v = 0; v < n; ++v) {
            SCIP_CONS* cons = nullptr;
            char consname[256];
            snprintf(consname, 255, "link2_%d_%d", i, v);
    ...
    // Σ_iv y[i,u,j,v] = 1 for all j,u
    for (int j = 0; j < n; ++j) {
        for (int u = 0; u < n; ++u) {
            SCIP_CONS* cons = nullptr;
            char consname[256];
            snprintf(consname, 255, "link3_%d_%d", j, u);
            SCIP_CALL_ABORT(SCIPcreateConsBasicLinear(scip, &cons, consname, 0, nullptr, nullptr, 1.0, 1.0));
            for (int i = 0; i < n; ++i) {
                for (int v = 0; v < n; ++v) {
                    SCIP_CALL_ABORT(SCIPaddCoefLinear(scip, cons, y.at({i, u, j, v}), 1.0));
    ...
    // Σ_ij y[i,u,j,v] = 1 for all u,v
    for (int u = 0; u < n; ++u) {
        for (int v = 0; v < n; ++v) {
            SCIP_CONS* cons = nullptr;
            char consname[256];
            snprintf(consname, 255, "link4_%d_%d", u, v);
            SCIP_CALL_ABORT(SCIPcreateConsBasicLinear(scip, &cons, consname, 0, nullptr, nullptr, 1.0, 1.0));
            for (int i = 0; i < n; ++i) {
                for (int j = 0; j < n; ++j) {
                    SCIP_CALL_ABORT(SCIPaddCoefLinear(scip, cons, y.at({i, u, j, v}), 1.0));
 ...
 * Corrected formula matching Python M4 implementation
 */
...
static SCIP_DECL_RELAXEXEC(relaxExecVolume) {
    ...
    VolumeResult res = solve_m4_volume_relax(data->problem, fixed, data->relaxation_info, data->last_dual, data->formulation_name);
 ...
 */
int solve_qap_m4(const std::string& instance_file, 
    ...
    SCIP_CALL_ABORT(SCIPcreateProb(scip, "QAP_M4", nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr));
 ...
 * 
 * Usage: m4_solver <instance.dat> [options]
...
int main(int argc, char** argv) {
    ...
    int result = solve_qap_m4(instance_file, warmstart_file, time_limit, threads, log_output,
