...
VOL_dvector& VOL_dvector::operator=(const VOL_dvector& w) {
   ...
   std::memcpy(v, w.v, wsz * sizeof(double));
   return *this;
}
...
VOL_ivector& VOL_ivector::operator=(const VOL_ivector& w) {
   ...
   std::memcpy(v, w.v, wsz * sizeof(int));
   return *this;
}
...
double VOL_dual::ascent(const VOL_dvector& v, const VOL_dvector& last_u) const 
{
   const int nc = u.size();
   int i;
   double asc = 0.0;
...
VOL_problem::read_params(const char* filename) 
{
   ...
   if (!infile) {
      std::cout << "\rFailure to open file: " << filename  << std::endl;
      abort();
   }
...
VOL_problem::set_default_parm()
{
   ...
   // Print only iteration info by default (single-line progress)
   parm.printflag = 1;
   parm.printinvl = 50;
			...
			const VOL_dual& dual)
{
   ...
   // Single-line, updating output for progress (std::cout version)
   std::cout << "\riter=" << iter
             << " time=" << std::setfill('0') << std::setw(2) << hours << ":"
             << std::setw(2) << minutes << ":" << std::setw(2) << seconds
             << " L=" << std::fixed << std::setprecision(6) << dual.lcost
             << " P=" << std::fixed << std::setprecision(6) << pstar.value
             << " infeas=" << std::fixed << std::setprecision(6) << indc.v2
             << " asc=" << std::fixed << std::setprecision(6) << indc.asc
             << " vmax=" << std::fixed << std::setprecision(6) << pstar.viol
...
int VOL_problem::solve(VOL_user_hooks& hooks, const bool use_preset_dual) 
{
   ...
   int ascent_first_check = VolMax(parm.ascent_first_check,
					 parm.ascent_check_invl);

   auto minimum_rel_ascent = 0.001;
   ...
   for (iter_ = 1; iter_ <= parm.maxsgriters; ++iter_) {  // main iteration
      ...
      //    auto gap = dstar.lcost - lcost_sequence[k];
      //    if (gap < VolAbs(lcost_sequence[k]) * minimum_rel_ascent){
      //       if(hooks.on_small_improvement()) 
      //       {
      //          if (parm.printflag & 2) std::cout << "\r*****small improvement -> adjusting parameters              " << std::endl;
      //          alpha_ = parm.alphainit;
      //          lambda_ = parm.lambdainit;
      //          // pstar = primal;
      //          // dual = dstar;
      //          // dstar = dual;
      //          ascent_first_check = iter_ + parm.ascent_check_invl;
      //       }
      //       else if (gap < VolAbs(lcost_sequence[k]) * parm.minimum_rel_ascent) 
      //       {
      //          if (parm.printflag & 2) std::cout << "\rsmall improvement                                    " << std::endl;
      //          break;
      //       } else {
      //          minimum_rel_ascent = parm.minimum_rel_ascent;
      //       }
      //    }
      // }
      if (iter_ > ascent_first_check) {
         auto gap = dstar.lcost - lcost_sequence[k];
         if (gap < VolAbs(lcost_sequence[k]) * parm.minimum_rel_ascent){
            if (parm.printflag & 2) std::cout << "\rsmall improvement                                    " << std::endl;
            break;
         }
      }
