...
int main(int argc, char* argv[]) {
    ...
    double ncv_init = 0.2; // number of Lanczos vectors
    double tol_init = 10.0; // tolerance for Lanczos solver
    ...
    for (int i = 2; i < argc; ++i) {
        ...
        if (arg == "--time" && i + 1 < argc) {
            ...
            C = std::atoi(argv[++i]);
        } else if (arg == "--ncv-init" && i + 1 < argc) {
            ncv_init = std::atof(argv[++i]);
        } else if (arg == "--tol-init" && i + 1 < argc) {
            tol_init = std::atof(argv[++i]);
        } else {
    ...
    if (formulation == "formulation1") {
        ...
        // vol_problem.parm.ubinit = 224420;
        set_up_vol_problem_6(vol_problem, qap_data, fv);
        auto hook6 = new SDPVolumeHooks6(qap_data, vol_problem, eigen_solver);
        hook6->m_ncv = int(ncv_init * qap_data.n * qap_data.n); // set number of Lanczos vectors
        hook6->m_tol = tol_init;

        std::cout << "ncv: " << hook6->m_ncv << ", tol: " << hook6->m_tol << std::endl;
        hooks = hook6;
    } else if (formulation == "formulation7") {
        set_up_vol_problem_7(vol_problem, qap_data, fv);
        hooks = new SDPVolumeHooks7(qap_data, vol_problem, eigen_solver);
    } else {
    ...
    // set_up_volume_parameters(vol_problem, verbose);
    {
        ...
        vol_problem.parm.gap_rel_precision = 0.0001;
        vol_problem.parm.granularity = 0.0;
        
        vol_problem.parm.ascent_first_check = 1000;
        vol_problem.parm.ascent_check_invl = 1000;
