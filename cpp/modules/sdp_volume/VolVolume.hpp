// $Id: VolVolume.hpp 375 2013-11-22 15:46:16Z tkr $
...
public:
   ...
   VOL_dvector(const VOL_dvector &x) : v(0), sz(0)
   {
      ...
      /** Move constructor to steal storage. */
      VOL_dvector(VOL_dvector &&x) noexcept : v(x.v), sz(x.sz) { x.v = 0; x.sz = 0; }
...
public:
   ...
   VOL_ivector(const VOL_ivector &x)
   {
      ...
      /** Move constructor to steal storage. */
      VOL_ivector(VOL_ivector &&x) noexcept : v(x.v), sz(x.sz) { x.v = 0; x.sz = 0; }
   /** The destructor deletes the data array. */
   ~VOL_ivector()
   {
      delete[] v;
   ...
   inline void swap(VOL_ivector &w)
   {
      ...
      /** Move assignment swaps storage. */
      VOL_ivector &operator=(VOL_ivector &&w) noexcept { swap(w); return *this; }
...
public:
   ...
   VOL_primal(const VOL_primal &primal) : value(primal.value), viol(primal.viol), x(primal.x), v(primal.v) {}
   ~VOL_primal() {}
...
public:
   ...
   VOL_dual(const VOL_dual &dual) : lcost(dual.lcost), xrc(dual.xrc), u(dual.u) {}
   ~VOL_dual() {}
...
public:
   ...
   lfactor(const VOL_parms &parm, const double lambda, const int iter)
   {
      ...
      case green:
         ...
         if (parm.printflag & 4)
            std::cout << "\r      G: Consecutive Gs = " << cons << std::endl;
         if (cons >= parm.greentestinvl && lambda < 2.0)
         {
            lastgreeniter = lastyellowiter = lastrediter = iter;
            lambdafactor = 2.0;
            if (parm.printflag & 2)
               std::cout << "\r ---- increasing lamda to " << lambda * lambdafactor << " ----" << std::endl;
         }
         break;
      ...
      case yellow:
         ...
         if (parm.printflag & 4)
            std::cout << "\r      Y: Consecutive Ys = " << cons << std::endl;
      ...
      case red:
         ...
         if (parm.printflag & 4)
            std::cout << "\r      R: Consecutive Rs = " << cons << std::endl;
         if (cons >= parm.redtestinvl && lambda > eps)
         {
            lastgreeniter = lastyellowiter = lastrediter = iter;
            lambdafactor = 0.67;
            if (parm.printflag & 2)
               std::cout << "\r **** decreasing lamda to " << lambda * lambdafactor << " *****" << std::endl;
         }
         break;
      }
      return lambdafactor;
   ...
   print()
   {
      std::cout << "**** G= " << ngs << ", Y= " << nys << ", R= " << nrs << " ****" << std::endl;
      ngs = nrs = nys = 0;
   }
};
...
public:
   ...
   */
   ...
   /** Called when the improvement in the objective function is small.
       @return true if the algorithm should continue, false otherwise
   */
   virtual bool on_small_improvement() { return false; }
};
