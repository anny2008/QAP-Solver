...
public:
	...
	int iteration;
public:
	SDPVolumeHooks(const Problem& data, VOL_problem& vol_prob): qap_data(data), n(data.n), vol_problem(vol_prob), subproblem_solving_time(0.0), iteration(0) {};
	...
	virtual int post_solving() { return 0; }
};
...
public:
	...
	int post_solving() override;
	virtual bool on_small_improvement() override;
};
...
public:
	SDPVolumeHooks7(const Problem& data, VOL_problem& vol_prob, std::string eigen_solver);
	virtual bool set_fixed_variables(const FixedVariables &fv) override;
	virtual int solve_subproblem(const VOL_dvector&, const VOL_dvector&, double&, VOL_dvector&, VOL_dvector&, double&) override;
