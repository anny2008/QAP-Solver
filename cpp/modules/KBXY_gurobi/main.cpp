...
int main(int argc, char* argv[]) {
    ...
    if (!config_path.empty()) {
        ...
        try {
            f >> j;
            cfg = KBXYGurobiConfig::fromJson(j);
    ...
    //  if n > m, add dummy locations with zero flow and distance to make it square
    if (problem.n > problem.m) {
        int n = problem.n;
        int m = problem.m;
        auto new_F = std::vector<std::vector<double>>(n, std::vector<double>(n, 0.0));
        for (int u = 0; u < m; ++u) {
            for (int v = 0; v < m; ++v) {
                new_F[u][v] = problem.F[u][v];
            }
        }
        problem.F = new_F;
        problem.m = n;
    ...
    KBXYGurobiSolver solver(cfg);
    Solution solution = solver.solve(problem, instance_path);
    std::cout << "KBXY-Gurobi finished.\n"
