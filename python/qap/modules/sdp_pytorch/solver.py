...
class SDPPytorchSolver:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.solver_name = config.get("solver", "sdp_pytorch")
        ...
        config_device = config.get("device", "cuda")  # Device for PyTorch computations
        # self.device = torch.device(config_device) if torch.cuda.is_available() else torch.device("cpu")
    ...
    @classmethod
    def from_config_file(cls, config_path: str) -> "SDPPytorchSolver":
        """
        ...
        Returns:
            SDPPytorchSolver: Initialized solver
        """
    ...
    def solve(
        ...
        bundle_config = BundleMethodConfig(
            epsilon=1e-4,
            mu=1.0,
            t=1.0,
            Dt_point_strategy="qp",
            master_problem_solver="cvxpy",
            # device=self.device,
            max_iters=self.config.get("max_iters", 1000),
            max_time=self.config.get("time_limit", 1000),
            log_interval=self.config.get("log_interval", 10),
            max_null_steps=self.config.get("max_null_steps", 20),
        )
        oracle = RLT1Oracle(problem)
        # oracle = TestOracle4()
        # oracle = TestOracle3()
        bundle_method = BundleMethod(bundle_config, oracle)
        ...
        print(f"Solver finished in {solving_time:.2f}")
        print(f"Objective value: {bundle_method.f_best:.2f}")
        ...
        result = Solution(
            ...
            objective=bundle_method.f_best,
            lower_bound=bundle_method.f_best,
            time=elapsed_time,
        )
