...
class RLT1SDPSolver:
    """RLT1 formulation solver using SDP."""
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.solver_name = config.get("solver", "rlt1_sdp")
    ...
    @classmethod
    def from_config_file(cls, config_path: str) -> "RLT1SDPSolver":
        """
        ...
        Returns:
            RLT1SDPSolver: Initialized solver
        """
        with open(config_path, "r") as f:
            config = json.load(f)
        return cls(config)

    def _create_model_1(self, problem: Problem) -> tuple:
        """
        Create SDP model for RLT1 formulation.
        ...
        constraints.append(Q_mat >= 0)
        # # Q_mat[i*n + u, j*n + v] >= 0 if f_uv*d_ij + f_vu*d_ji > 0
        # for i in range(n):
        #     for j in range(i+1,n):
        #         for u in range(n):
        #             for v in range(n):
        #                 if flows[i, j] * distances[u, v] + flows[v, u] * distances[j, i] > 0:
        #                     constraints.append(Q_mat[i*n + u, j*n + v] >= 0)
        # for i in range(n):
        #     for u in range(n):
        #         constraints.append(Q_mat[i*n + u, i*n + u] >= 0)
        #         for j in range(n):
        #             if i != j:
        #                 constraints.append(Q_mat[i*n + u, j*n + u] == 0)
        #             if u != j:
        #                 constraints.append(Q_mat[i*n + u, i*n + j] == 0)
        ...
        # 
        # Objective function
        objective = cp.Minimize(cp.trace(Q_mat @ np.kron(distances, flows)))

        # Constraints
        # Assignment constraints
        for i in range(n):
            constraints.append(cp.sum([P[i*n + u] for u in range(n)]) == 1)
        for u in range(n):
            constraints.append(cp.sum([P[i*n + u] for i in range(n)]) == 1)
        
        #
        # PSD constraint [Q_mat P; P^T 1] >> 0
        #
        constraints.append(cp.bmat([[Q_mat, P],
                                    [P.T, np.ones((1, 1))]]) >> 0)
        ...
        for i in range(n):
            for j in range(n):
                if i != j:
                    constraints.append(cp.trace(Q[i, j]) == 0)
        ...
        for i in range(n):
            for j in range(n):
                if i != j:
                    constraints.append(cp.trace(Q[i, j] @ (np.ones((n, n)) - np.eye(n))) == 1)
        
        # trace Q_ii In = 1 for all i
        for i in range(n):
            constraints.append(cp.trace(Q[i, i] @ np.ones((n, n))) == 1)
    ...
    def _create_model(self, problem: Problem) -> tuple:
        """
        Create SDP model for RLT1 formulation.
        ...
        # 1. Create the vector of ones of size n-1
        e_nm1 = np.ones((n - 1, 1))
        
        # 2. Construct V (size: n x n-1) as defined in equation (2.8)
        # V = [ I_{n-1} ]
        #     [  -e^T   ]
        I_nm1 = np.eye(n - 1)
        V = np.vstack([I_nm1, -e_nm1.T])
        
        # 3. Compute the Kronecker product V \otimes V (size: n^2 x (n-1)^2)
        V_kron_V = np.kron(V, V)
        
        # 4. Construct Vhat (size: (n^2 + 1) x ((n-1)^2 + 1))
        # Top row: [1, 0]
        top_row = np.zeros((1, 1 + (n - 1)**2))
        top_row[0, 0] = 1.0
        
        # Bottom-left block: (1/n) * e_{n^2} (column vector of ones)
        bottom_left = np.ones((n**2, 1)) / n
        
        # Combine bottom blocks: [ (1/n)*e , V \otimes V ]
        bottom_row = np.hstack([bottom_left, V_kron_V])
        
        # Stack top row and bottom block vertically
        W = np.vstack([top_row, bottom_row])
        
        r_dim = (n - 1)**2 + 1
        R = cp.Variable((r_dim, r_dim), symmetric=True)

        constraints = [R >> 0]  # R must be positive semidefinite

        # 2. Define the lifted matrix Y via facial reduction
        Y = W @ R @ W.T

        # 3. Top-left element constraint (E_00 = 1)
        constraints.append(R[0, 0] == 1)
        constraints.append(Y >= 0)  # E_0i = 0 for i > 0

        # 4. Extract the gangster block Y_bar (size n^2 x n^2)
        Y_bar = Y[1:, 1:]
        
        # 5. Apply Gangster Constraints element-by-element
        for i in range(n):
            for j in range(n):
                # Slice out the (i, j)-th block of size n x n
                block = Y_bar[i*n:(i+1)*n, j*n:(j+1)*n]
                
                if i == j:
                    # Diagonal blocks: off-diagonal elements are 0
                    # We can zero out the upper triangle (excluding diagonal) 
                    # and lower triangle will match due to symmetry
                    for r in range(n):
                        for c in range(r + 1, n):
                            constraints.append(block[r, c] == 0)
                else:
                    # Off-diagonal blocks: diagonal elements are 0
                    for d in range(n):
                        constraints.append(block[d, d] == 0)

        
        # objective function
        # L = [0 0; 0 D \otimes F]
        L = np.zeros((1 + n**2, 1 + n**2))
        L[1:, 1:] = np.kron(flows, distances)
        LQ = W.T @ L @ W
        objective = cp.Minimize(cp.trace(R @ LQ))
        print(f"Objective matrix LQ shape: {LQ.shape}")
        ...
        return R, Y, constraints, objective
    ...
    def solve(
        ...
        # Create model
        R, Y, constraints, objective = self._create_model(problem)
        create_time = time.time() - start_time
        print(f"Model created in {create_time:.2f} seconds")
        ...
        if model.status in ["optimal", "optimal_inaccurate"]:
            ...
            trace_R = cp.trace(R)
            trace_Y = cp.trace(Y)
            print(f"Trace of R: {trace_R.value}")
            print(f"Trace of Y: {trace_Y.value}")
