...
def solve_quad_subprob_ipm(
    ...
    dual_infeas_width = jnp.clip(
                                jnp.max(dual_infeas_vals) - jnp.min(dual_infeas_vals),
                                100.0,
                                None
                            )
    omega_init = jnp.asarray([-1.01 * jnp.min(dual_infeas_vals)])
    omega_init = jnp.clip(omega_init, dual_infeas_width, None)
    ...
    def body_func(ipm_state: IPMState) -> IPMState:
        ...
        step_size = jnp.clip(
            -1.0 / jnp.min(step_size_denoms / step_size_numers),
            None,
            1.0
        )
        ...
        mu_next = jnp.clip(mu_next, None, ipm_state.mu)
...
def compute_lb_spec_est_ipm(
    ...
    zeta_init = g_2
    dual_infeas_vals = jnp.append(jnp.diag(T_init), zeta_init)
    dual_infeas_width = jnp.clip(jnp.max(dual_infeas_vals) - jnp.min(dual_infeas_vals), 100.0, None)
    omega_init = jnp.asarray([-1.01 * jnp.min(dual_infeas_vals)])
    omega_init = jnp.clip(omega_init, dual_infeas_width, None)
    T_init += omega_init * jnp.eye(k)
    zeta_init += omega_init
    ...
    def body_func(ipm_state: IPMState) -> IPMState:
        ...
        step_size = jnp.clip(-1.0 / jnp.min(step_size_denoms / step_size_numers), None, 1.0)
        step_size = 0.99 * lax.select(step_size <= 0.0, 1.0, step_size)
...
def solve_step_subprob(
    ...
    def body_func(state: SubprobStateStruct) -> SubprobStateStruct:
        ...
        S_eigvals = jnp.clip(S_eigvals, 0, None)    # numerical instability handling
        ...
        upsilon_next = b_ineq_mask * jnp.clip(b - z_next - (rho * y), 0.0, None)
...
def usbs(
    ...
    def cond_func(state: StateStruct) -> Array:
        ...
        ellapsed_time = np.round(state.curr_time - state.start_time, 2)

        # compute gap between primal and dual objectives
        dual_obj = jnp.dot(state.b, state.y)
        obj_gap = (state.primal_obj - dual_obj) / (dual_obj + 1e-8)
        # best_ub = state.best_ub
        
        # if best_ub is None or (callback_val is not None and callback_val > best_ub):
        #     best_ub = callback_val
            
        best_obj_gap = state.best_obj_gap if state.best_obj_gap is not None else -1
        jax.debug.print("\nt: {t} - ellapsed: {curr_time}s"
                        " - best_ub: {best_ub} - best_obj_gap: {best_obj_gap}"
                        ...
                        t=state.t,
                        curr_time=ellapsed_time,
                        pen_dual_obj=state.pen_dual_obj / (SCALE_C * SCALE_X),
                        cand_pen_dual_obj=state.cand_pen_dual_obj / (SCALE_C * SCALE_X),
                        lb_spec_est=state.lb_spec_est,
                        primal_obj=state.primal_obj / (SCALE_C * SCALE_X),
                        dual_obj=dual_obj / (SCALE_C * SCALE_X),
                        obj_ub=state.obj_ub / (SCALE_C * SCALE_X),
                        obj_gap=obj_gap,
                        infeas_gap=state.infeas_gap,
                        max_infeas=state.max_infeas,
                        callback_val=callback_val,
                        best_ub=state.best_ub,
                        best_obj_gap=state.best_obj_gap
                        )
    ...
    def body_func(state: StateStruct) -> StateStruct:
        ...
        cand_pen_dual_obj = jnp.dot(state.b, y_cand) + trace_ub*jnp.clip(cand_eigvals[0], 0, None)
        obj_ub = jnp.clip(
            jnp.dot(state.b, y_cand)
            + (trace_ub / trace_factor)*jnp.clip(cand_eigvals[0], 0, None),
            None,
            state.obj_ub)
        ...
        obj_gap /= 1.0 + jnp.clip(jnp.abs(primal_obj_next), jnp.abs(obj_ub), None) / (SCALE_C * SCALE_X)
        infeas = (jnp.clip(z_next - state.b, 0.0, None)
                  + (1.0 - state.b_ineq_mask) * jnp.clip(state.b - z_next, 0.0, None))
        ...
        dual_obj = jnp.dot(state.b, y_next)
        best_ub = state.best_ub
        # best_obj_gap = min(dual_obj, state.best_obj_gap)
        best_obj_gap = jnp.clip(state.best_obj_gap, None, dual_obj)
        # # jax.debug.print("best_ub: {best_ub} - best_obj_gap: {best_obj_gap}",
        # #                 best_ub=best_ub ,
        # #                 best_obj_gap=best_obj_gap)
    ...
    init_pen_dual_obj += trace_ub * jnp.clip(init_eigvals[0], 0, None)
    ...
    return SDPState(
        ...
        SCALE_A=sdp_state.SCALE_A,
        best_ub=-1,
        best_obj_gap=final_state.best_obj_gap)
