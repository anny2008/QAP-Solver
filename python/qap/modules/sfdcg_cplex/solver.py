...
class BranchAndPriceSFD:
    ...
    def solve(self, problem, subgraphs, warmstart):
        ...
        warmstart:   dict (i,u) → 1 for initial solution
        ...
        while tree.has_open_nodes():
            ...
            # adapt warmstart with branching fixes
            warmstart_to_use = None
            if warmstart is not None:
                # check if warm start assignments are compatible with branching fixes at this node
                # if not, simple add the varible that is fixed to 1 
                warmstart_to_use = warmstart.copy()
                for (i,u), val in node.fixed_x.items():
                    if val == 1 and warmstart.get((i,u), 0) < 0.5:
                        warmstart_to_use[(i,u)] = 1.0
                    
            result = self._solve_node(problem, subgraphs, node, warmstart=warmstart_to_use)
            if result is None:
                if self.verbose:
            ...
            lb, x_vals = result
            if self.verbose:
                print(f"Node {node.id} LP relaxation: LB={lb:.4f}")
        ...
        print(f"\n=== SOLUTION: obj={self.best_obj:.4f} time={elapsed:.2f}s nodes={nodes_visited} ===")
        return Solution(
            instance="",
    ...
    def _extract_columns_from_warmstart(self, problem, subgraphs, warmstart):
        ...
        for k, subgraph in subgraphs.items():
            Fk, arcs, nodes = subgraph
            for u, v in arcs:
                i = -1
                j = -1
                for ii,uu in warmstart.keys():
                    if uu == u:
                        i = ii
                    if uu == v:
                        j = ii
                if i != -1 and j != -1:
                    columns.append((k, (k,i,j)))
                else:
                    print(f"Warning: Warmstart solution does not assign machines {u} and {v} to any locations, skipping column for subgraph {k} arc ({u},{v})")
        return columns
    ...
    def _solve_node(self, problem, subgraphs, node, warmstart=None):
        ...
        """
        print(f"Solving node {node.id} with {len(node.fixed_x)} fixed variables...")
        ...
        for it in range(self.max_iterations):
            ...
            # Duals
            dual_lambda, dual_theta = rmp.get_duals()

            # Pricing
            branching_fixed_x = node.fixed_x
            negative_cols, most_negative_cols, most_negative_rc = pricing.price(rmp.Omega, dual_lambda, dual_theta)

            if not most_negative_cols:
            ...
            # Add columns
            n_added = 0
            for k in most_negative_cols:
                for (rc, col) in most_negative_cols[k]:
                    if rmp.add_column(k, col):
                        n_added += 1
            
            if self.verbose:
                print(f"    Added {n_added} columns with negative reduced cost")
                print(f"    Number of columns in RMP: {len(rmp.e_index)}")
